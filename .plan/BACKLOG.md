...
- [ ] **Homepage leads with the pillars, not with the reason for them** — a visitor meets three parallel boxes (`src/pages/vision/index.astro:1`, "Core Pillars of European Integration") before any argument for why Europe must act at all. Put the demographic frame above them: aging society → the pension model breaks → we design the replacement now. (High, 5)
- [ ] **The three pillars are presented as peers; they are layers with an order** — language/economy makes the digital state possible, which makes robotics possible. Present them stacked, with the dependency visible. Touches `src/pages/vision/index.astro`, `src/pages/index.astro`. (High, 5)
- [ ] **"Who captures the gains" is not visible anywhere** — the distribution question runs through every proposal (automation replacing the labour pensions depend on is worthless if the returns are enclosed). Make it a cross-cutting principle shown on each proposal, not a buried section. (High, 3)
- [ ] **Direct democracy, predictive healthcare and autonomous transport are listed as peers of the pillars** — they are components of a layer. Regroup under the layer they belong to (`src/pages/vision/index.astro`, "Additional Focus Areas"). (Medium, 3)
